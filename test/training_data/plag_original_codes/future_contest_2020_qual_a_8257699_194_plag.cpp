...
A - ロボットの誘導Editorial
// ソースコードの引用元 :
https://atcoder.jp/contests/future-contest-2020-qual/submissions/8257699
// 提出ID : 8257699
...
int main(void) {
  ...
  for (int r = 0; r < m; r++) { //ここあとで帰る
    ...
    while (!q.empty()) {
      ...
      // if(r==5)cout<<now.first<<" "<<now.second<<" "<<gx<<" "<<gy<<endl;
      if (now.first == gx && now.second == gy) {
        // if(r==5)cout<<"AAA"<<endl;
        //ゴール
        break;
      }
      if (info[now.second][now.first] > 0) {
        // if(r==5)cout<<"BBB"<<endl;
        lastx = now.first;
        lasty = now.second;
        //既に書かれているから終了
        break;
