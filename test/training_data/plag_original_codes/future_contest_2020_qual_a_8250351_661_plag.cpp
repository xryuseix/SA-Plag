...
A - ロボットの誘導Editorial
// ソースコードの引用元 :
https://atcoder.jp/contests/future-contest-2020-qual/submissions/8250351
// 提出ID : 8250351
...
int main(void) {
        ...
        if (info[i][j] > 0) {
          ...
          if (j - 1 >= 0 && (grid[i][j - 1] == 0 || grid[i][j - 1] == 1)) {
            info[i][j - 1] = 2;
                     ...
                     (grid[i][j - 1 + n] == 0 || grid[i][j - 1 + n] == 1)) {
            info[i][j - 1 + n] = 2;
          }
          if (j + 1 < n && (grid[i][j + 1] == 0 || grid[i][j + 1] == 1)) {
            info[i][j + 1] = 3;
          } else if (j + 1 >= n &&
                     (grid[i][j + 1 - n] == 0 || grid[i][j + 1 - n] == 1)) {
            info[i][j + 1 - n] = 3;
  ...
  //     for(int j=0;j<n;j++){
  //         if(grid[i][j]==2){
  //             if(i-1>=0)info[i-1][j]=4;
  //             else info[i-1+n][j]=4;
  //             if(i+1<n)info[i+1][j]=2;
  //             else info[i+1-n][j]=2;
  //             if(j+1<n)info[i][j+1]=1;
  //             else info[i][j+1-n]=1;
  //             if(j-1>=0)info[i][j-1]=3;
  //             else info[i][j-1+n]=3;
  //         }
  //     }
  // }
