...
A - ロボットの誘導Editorial
// ソースコードの引用元 :
https://atcoder.jp/contests/future-contest-2020-qual/submissions/8258167
// 提出ID : 8258167
...
int main(void) {
  ...
  for (int i = 1; i < n - 1; i++) {
    for (int j = 1; j < n - 1; j++) {
      if (info[i - 1][j] == info[i + 1][j] && info[i][j] == info[i + 1][j] &&
          info[i][j + 1] != 2 && info[i][j - 1] != 4 && grid[i][j] != 1 &&
          (info[i][j] == 1 || info[i][j] == 3)) {
        info[i][j] = 0;
      }
    }
  }
  for (int i = 1; i < n - 1; i++) {
    for (int j = 1; j < n - 1; j++) {
      if (info[i][j - 1] == info[i][j + 1] && info[i][j] == info[i][j + 1] &&
          info[i + 1][j] != 1 && info[i - 1][j] != 3 && grid[i][j] != 1 &&
          (info[i][j] == 2 || info[i][j] == 4)) {
        info[i][j] = 0;
      }
    }
  }
